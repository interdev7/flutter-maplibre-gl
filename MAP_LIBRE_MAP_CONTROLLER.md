...
15. [Кэш, офлайн-регионы и телеметрия](#15-кэш-офлайн-регионы-и-телеметрия)
...
## 15. Кэш, Офлайн-регионы и Телеметрия

### Фоновый кэш (Ambient Cache)

Карта автоматически кэширует тайлы по мере просмотра (по умолчанию до 50 МБ). Управлять этим кэшем можно через методы контроллера:
...
### Офлайн-регионы (Offline Regions)

Вы можете программно загрузить участки карты для гарантированной работы без интернета. Эти методы вызываются как глобальные функции (вне контроллера).
...
// 1. Описываем регион (границы, стиль, диапазоны зума)
final definition = OfflineRegionDefinition(
  bounds: LatLngBounds(
    southwest: LatLng(55.5, 37.3),
    northeast: LatLng(55.9, 37.9),
  ),
...
// 2. Запускаем скачивание с отслеживанием прогресса
final region = await downloadOfflineRegion(
  definition,
  metadata: {'name': 'Moscow Region'},
...
**Управление скачанными регионами:**

```dart
// Получить список всех скачанных регионов
final regions = await getListOfRegions();
for (var r in regions) {
  print('Регион ID: ${r.id}');
}

// Обновить метаданные региона
await updateOfflineRegionMetadata(region.id, {'name': 'Updated Name'});

// Удалить регион из памяти устройства
await deleteOfflineRegion(region.id);
```

**Глобальный оффлайн-режим:**

```dart
await setOffline(true); // Перевести все карты в приложении в принудительный оффлайн-режим
```

### Телеметрия и настройки производительности
...
await controller.forceOnlineMode(); // принудительно запретить использовать кэш карты
```
